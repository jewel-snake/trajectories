...
fn law(instance: Vector6d) -> Vector6d {
    let r = (instance.x.powi(2)+instance.y.powi(2)).sqrt();
    let fi_s = fi_b(r,instance.z) + fi_d(r,instance.z) + fi_h(r,instance.z);
    let fi_s_dr = fi_b(r+1.0,instance.z) + fi_d(r+1.0,instance.z) + fi_h(r+1.0,instance.z);
    let fi_s_dz = fi_b(r,instance.z+1.0) + fi_d(r,instance.z+1.0) + fi_h(r,instance.z+1.0);
    Vector6d {
        x:0.0,y:0.0,z:0.0,
        m:(fi_s_dr-fi_s)/1.0,
        n:(fi_s_dr-fi_s)/1.0,
...
fn fi_b(r: f64,z:f64) -> f64{
    let rb = (z.powi(2)+r.powi(2)).sqrt();
    -142.0/(rb.powi(2)+249.7_f64.powi(2)).sqrt()
}
...
fn fi_h(r: f64,z:f64) -> f64{
    let rb = (r.powi(2)+z.powi(2)).sqrt();
    -24572.0/64300_f64*((64300_f64+(64300_f64.powi(2)+rb.powi(2)).sqrt())/rb).ln()
}

fn dm_dt(instance: Vector6d) -> Vector6d {
    let r = instance.position_mag();
    let accel = law(instance)*-2.325e7*1.022018688e-6*1.022018688e-6/100.0*1.1;//* 1.0749776097859195e-11* 1.022018688e-6_f64.powi(2)*2.325e7_f64/1.0
    //print!("{}",accel.velocity_mag());
    Vector6d {
        x: instance.m,
        y: instance.n,
        z: instance.k,
        m: accel.m * (instance.x/r),
        n: accel.n * (instance.y/r),
        k: accel.k
    }
}
